...
void sol(){
    ...
    if (a == 1) {
        string timHoTen;
        cin.ignore(1e9, '\n');
        getline(cin, timHoTen);

        vector<string> foundMaSVs;
        for (const auto &sv : danhSachSV) {
            if (sv.getHoTen() == timHoTen) {
                foundMaSVs.push_back(sv.getMaSV());
            }
        }
        ...
        if (foundMaSVs.empty()) {
            ...
            for (const auto &maSV : foundMaSVs) {
                cout << maSV << " ";
            }
        }
        cout << endl;
